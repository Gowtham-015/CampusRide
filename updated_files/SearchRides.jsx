...
import { searchUniversities } from '../data/bangaloreUniversities.js';
...
function PlaceInput({ placeholder, value, onSelect, icon, universitiesOnly = false }) {
        ...
        <div style={{ position:'absolute', top:'100%', left:0, right:0, zIndex:999, background:'#1a1a2e', border:'1px solid #333', borderRadius:8, maxHeight:220, overflowY:'auto', boxShadow:'0 8px 24px #0008' }}>
          {results.map((r, i) => (
            <div key={i} onClick={() => pick(r)}
              ...
              {universitiesOnly ? '🎓' : '📍'} {r.label}
              {universitiesOnly && r.type && <span style={{ fontSize:11, color:'#888', marginLeft:8 }}>({r.type})</span>}
            </div>
          ))}
        </div>
      )}
...
export default function SearchRides({ navigate }) {
  ...
  const applyFilters = useCallback(async () => {
    ...
    // Only show active rides — hide completed and cancelled
    let result = source.filter(r => r.status === 'active' || r.status === 'in-progress');
    ...
    <div className="page-wrap fade-up">
      ...
      {!kycApproved && kycPending && (
        <div style={{ background:'rgba(245,166,35,0.10)', border:'1.5px solid rgba(245,166,35,0.45)', borderRadius:12, padding:'14px 18px', marginBottom:20, display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
          <div>
            ...
            <div style={{ color:'#888', fontSize:13 }}>Your documents are being reviewed. You can book rides once approved.</div>
          </div>
      ...
      {!kycApproved && !kycPending && (
        <div style={{ background:'rgba(245,60,60,0.10)', border:'1.5px solid rgba(245,80,80,0.45)', borderRadius:12, padding:'14px 18px', marginBottom:20, display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
          <div>
            ...
            <div style={{ color:'#888', fontSize:13 }}>Complete KYC verification to book rides. Clicking "Book Seat" will redirect you to KYC.</div>
          </div>
        ...
        <div className="card-body">
          ...
          <div className="grid-2 mb-12">
            <div className="field" style={{ marginBottom:0 }}>
              <label>FROM (College/University pickup) *</label>
              <PlaceInput placeholder="Type college or university name..." value={fromPlace.label} icon="🟢"
                onSelect={(p) => setFromPlace({ label:p.label, lat:p.lat, lng:p.lng })} universitiesOnly={true} />
            </div>
            <div className="field" style={{ marginBottom:0 }}>
      ...
      {!hasSearched && !searching && (
        <div className="empty-state" style={{ paddingTop:40 }}>
          ...
          <div className="empty-sub mt-8">Enter a <strong>pickup area</strong> above and click <strong>Apply Filters</strong> to see available rides.</div>
        </div>
      )}
          ...
          {rides.length === 0 ? (
            <div>
              <div className="empty-state">
                ...
                <div className="empty-sub mt-8">Try a wider search radius or a different area.</div>
