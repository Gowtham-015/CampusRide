...
// ADMIN: Remove/clear KYC verification so the user can be re-reviewed later.
exports.revokeKyc = async (req, res) => {
  try {
    const { userId, reason } = req.body;
    if (!reason || typeof reason !== 'string' || !reason.trim()) {
      return res.status(400).json({ message: 'A reason is required to revoke KYC verification.' });
    }
    ...
    user.kycData.rejectReason = '';

    user.kycData.reviewedAt = null;
    ...
    await Notification.create({
      userId,
      type: 'kyc_revoked',
      title: 'KYC verification was revoked',
      body: note,
      read: false,
    });
    ...
    if (io) {
      io.to(`user-${userId}`).emit('app-notification', {
        type: 'kyc_revoked',
        title: 'KYC verification was revoked',
        body: note,
      });
    ...
    res.json({ message: 'KYC revoked', user: fresh });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
