...
exports.createRide = async (req, res) => {
  try {
    ...
    // Enforce college/university location in pickup OR drop
    const collegeKeywords = ['college','university','institute','iit','nit','bits','vit','dsu','rnsit','rns','manipal','amrita','pes','jain','christ','msrit','bms','sit','rvce','bvb','campus','polytechnic','academy','school of'];
    const pickupLabel = (pickup.label || '').toLowerCase();
    const dropLabel   = (drop.label   || '').toLowerCase();
    const hasCollege  = collegeKeywords.some(k => pickupLabel.includes(k) || dropLabel.includes(k));
    if (!hasCollege) {
      return res.status(400).json({
        message: 'At least one location (pickup or drop) must be a college or university address. CampusRide is exclusively for campus rides.'
      });
    }
...
exports.pickupPassenger = async (req, res) => {
  try {
    ...
    // Enforce: provider can only start AT or AFTER scheduled time
    // Fix: use ride.date as a date string and combine with ride.time string
    // to avoid UTC/IST timezone offset issues (server runs UTC, users are in IST)
    const rideDateStr = new Date(ride.date).toISOString().split('T')[0]; // "2026-03-30"
    const rideTimeStr = ride.time || '00:00';                            // "14:38"
    // Parse as local time by using Date constructor with explicit parts
    const [year, month, day] = rideDateStr.split('-').map(Number);
    const [h, m] = rideTimeStr.split(':').map(Number);
    // Build the scheduled datetime in UTC by offsetting IST (+5:30)
    // Instead of setHours (which uses local server TZ), build UTC manually
    const IST_OFFSET_MS = 5.5 * 60 * 60 * 1000; // 5h30m in ms
    const rideDateTime  = new Date(Date.UTC(year, month - 1, day, h, m, 0) - IST_OFFSET_MS);
    const now = new Date();
    if (now < rideDateTime) {
      const diffMins = Math.ceil((rideDateTime - now) / 60000);
      const hours    = Math.floor(diffMins / 60);
      const mins     = diffMins % 60;
      const timeStr  = hours > 0 ? `${hours}h ${mins}m` : `${mins} minutes`;
      return res.status(400).json({
        message: `Ride cannot start yet. Scheduled for ${rideTimeStr} on ${new Date(ride.date).toLocaleDateString('en-IN', { day:'numeric', month:'short' })}. Starts in ${timeStr}.`,
        scheduledTime: rideDateTime,
