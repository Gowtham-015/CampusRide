...
const ADMIN_NAV_LINKS = [
  ...
  { tab: "Rides", label: "Rides", icon: "🚗" },
  { tab: "Bookings", label: "Bookings", icon: "📋" },
  { tab: "KYC Review", label: "KYC Review", icon: "🪪" },
  { tab: "Incidents", label: "Incidents", icon: "⚠️" },
];

export default function Navbar({ navigate, currentPage, adminTab = "Overview" }) {
    ...
    const load = () => {
      import('../services/api.js').then(api => {
        api.getRideRequests().then(data => {
          setRideRequestCount((data || []).filter(b => b.status === 'pending').length);
        }).catch(() => {});
      });
    };
    load();
  ...
  const isAdminNavActive = (tab) => currentPage === "admin" && adminTab === tab;
    ...
    <>
      <nav className="navbar">
        <div className="nav-inner">
          <button className="nav-logo" onClick={() => (isAdmin ? goAdminTab("Overview") : go("dashboard"))}>
            ...
            {isAdmin ? (
              <>
                {ADMIN_NAV_LINKS.map((l) => (
                  <button
                    key={l.tab}
                    type="button"
                    className={`nav-link ${isAdminNavActive(l.tab) ? "active" : ""}`}
                    onClick={() => goAdminTab(l.tab)}
                  >
                ...
                <button
                  type="button"
                  className={`nav-link ${currentPage === "admin-settings" ? "active" : ""}`}
                  onClick={goSettings}
                >
              ...
              links.map((l) => (
                ...
                >
                  <span className="nl-icon">{l.icon}</span>
                  {l.label}
                  {l.key === "notifications" && rideRequestCount > 0 && (
                    <span
                      style={{
                        marginLeft: 4,
                        background: "#ff4444",
          ...
          <div className="nav-right">
            {!isAdmin && (
              ...
              >
                <span className="nav-inbox-icon" aria-hidden>
                  🔔
                </span>
                {inboxUnread > 0 && (
                  <span className="nav-inbox-badge">{inboxUnread > 99 ? "99+" : inboxUnread}</span>
                )}
              </button>
            )}
            ...
            <>
              {ADMIN_NAV_LINKS.map((l) => (
                <button
                  key={l.tab}
                  type="button"
                  className={`mobile-link ${isAdminNavActive(l.tab) ? "active" : ""}`}
                  onClick={() => goAdminTab(l.tab)}
                >
              ...
              <button
                type="button"
                className={`mobile-link ${currentPage === "admin-settings" ? "active" : ""}`}
                onClick={goSettings}
              >
