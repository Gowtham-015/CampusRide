...
export const revokeKyc      = (body)   => req('/kyc/admin/revoke',             { method: 'POST', body: JSON.stringify(body) });
...
export const deleteAdminUser  = (id, reason)  =>
  req(`/admin/users/${id}`, { method: 'DELETE', body: JSON.stringify({ reason }) });

export const getNotifications       = () => req('/notifications');
export const getUnreadNotificationCount = () => req('/notifications/unread-count');
export const markNotificationRead   = (id) => req(`/notifications/${id}/read`, { method: 'PUT' });
export const markAllNotificationsRead = () => req('/notifications/read-all', { method: 'PUT' });
